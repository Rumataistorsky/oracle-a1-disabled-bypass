...
def cover(c):
    ...
    ny = block(left_x, ny, 'ОДОМЕТР / ODOMETER', [
        ('За договором',        '47 305 км  ·  16.09.2026'),
        ('Перегін від дилера',  None),
    ...
    c.setFont('DJ', 7.5)
    c.setFillGray(0.35)
    c.drawString(left_x, ny + 2, 'Перегін від дилера в GPS не потрапив (~16 км). Вписати першим рядком журналу.')
    c.setFillGray(0)
