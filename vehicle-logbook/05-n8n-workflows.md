...
Cron (0 * * * *)
  ...
  → Function  нормалізація + правила класифікації
...
[ Постачальник ]  [ Особисте ]
```

Після вибору «Оцінка» або «Обʼєкт у роботі» бот питає один рядок:
**«що саме?»** — текст іде в `trips.purpose`. Це і є поле «мета» для CRA,
